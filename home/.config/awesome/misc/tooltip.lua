...
local wifi_text = wibox.widget{
    markup = helpers.colorize_text("WiFi", beautiful.xcolor9),
...
local batt_text = wibox.widget{
    markup = helpers.colorize_text("Battery", beautiful.xcolor9),
    font = beautiful.font_name .. "8",
    valign = "center",
    widget = wibox.widget.textbox
}
...
local music_text = wibox.widget{
    markup = helpers.colorize_text("Nothing Playing", beautiful.xcolor9),
    font = beautiful.font_name .. "8",
    valign = "center",
    widget = wibox.widget.textbox
}
...
local music_artist = wibox.widget{
    markup = helpers.colorize_text("No Artist", beautiful.xcolor9),
...
playerctl:connect_signal("metadata", function(_, title, artist, album_path, __, ___, ____)
    ...
    music_artist:set_markup_silently(helpers.colorize_text(artist, beautiful.xcolor9))
end)

playerctl:connect_signal("playback_status", function(_, playing, __)
    if playing then
        music_text:set_markup_silently(helpers.colorize_text("Now Playing", beautiful.xcolor9))
        music_status:set_markup_silently("")
    else
        music_text:set_markup_silently(helpers.colorize_text("Music", beautiful.xcolor9))
        music_status:set_markup_silently("")
    end
end)
...
awful.placement.bottom_right(stats_tooltip, {honor_workarea = true, margins = {bottom = beautiful.useless_gap, right = dpi(50)}})
...
local date_day = wibox.widget{
    font = beautiful.font_name .. "8",
    format = helpers.colorize_text("%A", beautiful.xcolor9),
...
local time_boxed = create_boxed_widget(time, dpi(110), dpi(50), true)
local weather_boxed = create_boxed_widget(weather, dpi(60), dpi(70), true)
...
awful.placement.bottom_right(cal_tooltip, {honor_workarea = true, margins = {bottom = beautiful.useless_gap, right = dpi(12)}})
